...
static class DepVSpalte5
{
    ...
    static readonly Dictionary<string,double?[]> ClassValues=new(){
        ["Glühverlust|M%"]=new double?[]{3,3,5,10},["TOC|M%"]=new double?[]{1,1,3,6},["BTEX|mg/kg"]=new double?[]{6,null,null,null},["Σ PCB 7|mg/kg"]=new double?[]{1,null,null,null},
        ["MKW C10-C40|mg/kg"]=new double?[]{500,null,null,null},["PAK 16|mg/kg"]=new double?[]{30,null,null,null},["Extrahierbare lipophile Stoffe|M%"]=new double?[]{.1,.4,.8,4},
        ["DOC|mg/l"]=new double?[]{50,50,80,100},["Phenole|µg/l"]=new double?[]{100,200,50000,100000},["Arsen|µg/l"]=new double?[]{50,200,200,2500},["Blei|µg/l"]=new double?[]{50,200,1000,5000},["Cadmium|µg/l"]=new double?[]{4,50,100,500},
        ["Kupfer|µg/l"]=new double?[]{200,1000,5000,10000},["Nickel|µg/l"]=new double?[]{40,200,1000,4000},["Quecksilber|µg/l"]=new double?[]{1,5,20,200},["Zink|µg/l"]=new double?[]{400,2000,5000,20000},
        ["Chlorid|mg/l"]=new double?[]{80,1500,1500,2500},["Sulfat|mg/l"]=new double?[]{100,2000,2000,5000},["Cyanide|mg/l"]=new double?[]{.01,.1,.5,1},["Fluorid|mg/l"]=new double?[]{1,5,15,50},["Barium|mg/l"]=new double?[]{2,5,10,30},
        ["Chrom gesamt|µg/l"]=new double?[]{50,300,1000,7000},["Molybdän|µg/l"]=new double?[]{50,300,1000,3000},["Antimon|µg/l"]=new double?[]{6,30,70,500},["Selen|µg/l"]=new double?[]{10,30,50,700},["Gesamtgehalt an gelösten Feststoffen|mg/l"]=new double?[]{400,3000,6000,10000}
    };
    ...
    public static string Class(List<Reading> readings,int sample){var classes=new[]{"DK 0","DK I","DK II","DK III"};var comparable=readings.Where(r=>!r.Ignored[sample]&&r.Values[sample]!=null&&ClassValues.ContainsKey(r.Name+"|"+r.Unit)).ToList();if(comparable.Count==0)return "Keine Werte";for(int index=0;index<classes.Length;index++)if(comparable.All(r=>ClassValues[r.Name+"|"+r.Unit][index] is not double max||r.Values[sample]<=max))return classes[index];return "> DK III";}
    public static string Overall(List<Reading> readings,int sample){var comparable=readings.Where(r=>!r.Ignored[sample]&&r.Values[sample]!=null&&Limit(r) is {Orientation:false}).ToList();if(comparable.Count==0)return "keine Werte";return comparable.All(r=>Result(r,sample)=="eingehalten")?"eingehalten":"überschritten";}
    public static string Drivers(List<Reading> readings,int sample){var matches=readings.Where(r=>Result(r,sample)=="überschritten").Take(4).Select(r=>$"{r.Name} {r.Texts[sample]}{(r.Unit.Length==0?"":" "+r.Unit)}").ToArray();return matches.Length==0?"":"Spalte 5 überschritten: "+string.Join(", ",matches);}
}
...
class MainForm:Form
{
    ...
    public MainForm(){Text="EBV Scan 1.6.1";Width=1280;Height=780;AllowDrop=true;material.Items.AddRange(EBVData.Tables.Keys.ToArray());material.SelectedItem="Boden";
